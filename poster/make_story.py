...
for y in range(H):
    v = 0
    if y < 640:
        v = int(200 * (1 - y / 640) ** 1.25)
    if y > 1380:
        v = max(v, int(205 * ((y - 1380) / (H - 1380)) ** 1.4))
    grad.putpixel((0, y), v)
...
if os.path.exists("logo.png"):
    ...
    lg.thumbnail((300, 260), Image.LANCZOS)
    # soft glow behind logo
    glow = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    gd = ImageDraw.Draw(glow)
    gx, gy = W // 2, 100 + lg.height // 2
    gd.ellipse((gx - 150, gy - 130, gx + 150, gy + 130), fill=(70, 130, 230, 90))
    glow = glow.filter(ImageFilter.GaussianBlur(40))
    img.alpha_composite(glow)
    img.alpha_composite(lg, ((W - lg.width) // 2, 95))
    logo_h = lg.height
    d = ImageDraw.Draw(img)

# ---------- headline ----------
text_center(430, "برد شیرین نوجوانان اتحاد ایلام", font("FD-Black", 78), WHITE, glow=GLOW_B)
text_center(565, "مقابل تیم هاکان کرمانشاه", font("FD-Bold", 54), ICE)

# divider
d.line((W/2 - 170, 668, W/2 + 170, 668), fill=SILVER + (230,), width=3)
d.ellipse((W/2 - 7, 661, W/2 + 7, 675), fill=SILVER)
...
# ---------- score plate ----------
cx = 620
cy = 985 if PHOTO_MODE else 985
plate_w = 640
plate_h = 290 if PHOTO_MODE else 290
...
f_score = ImageFont.truetype("fonts/Vazirmatn-Black.ttf", 120)
...
if PHOTO_MODE:
    ...
    d = ImageDraw.Draw(img)
    slogan_y, sub_y = 1690, 1805
    slogan_size, sub_size = 70, 32
else:
...
img.convert("RGB").save("story_final.jpg", quality=93)
print("saved story_final.jpg")
